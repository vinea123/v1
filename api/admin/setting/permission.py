...
class PermissionHandler:
   ...
   #Read JSON Body
    ...
    def _get_id(self):
        ...
        if len(parts) == 3 and parts[0] == "admin" and parts[1] == "permission" and parts[2].isdigit():
            return int(parts[2])
        return None
    ...
    def handle_post(self):
        if self.request_handler.path != "/admin/permission":
            return 404, {"Content-Type": "application/json"}, json.dumps({"error": "Invalid URL"}).encode()
        ...
        try:
            ...
            cursor.execute("SELECT * FROM permission WHERE id = %s", (inserted_id,))
            row = cursor.fetchone()
    ...
    def handle_put(self):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("UPDATE permission SET name = %s WHERE id = %s", (name, permission_id))
            conn.commit()
    ...
    def handle_delete(self):
        ...
        try:
            ...
            # Delete the record
            cursor.execute("DELETE FROM permission WHERE id = %s", (permission_id,))
            conn.commit()
