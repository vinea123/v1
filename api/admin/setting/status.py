...
class StatusHandler:
   ...
   #Read JSON Body
    ...
    def _get_id(self):
        ...
        if len(parts) == 3 and parts[0] == "admin" and parts[1] == "status" and parts[2].isdigit():
            return int(parts[2])
        return None
    ...
    def handle_post(self):
        if self.request_handler.path != "/admin/status":
            return 404, {"Content-Type": "application/json"}, json.dumps({"error": "Invalid URL"}).encode()
        ...
        try:
            ...
            cursor.execute("INSERT INTO status (name) VALUES (%s)", (name,))
            conn.commit()
            inserted_id = cursor.lastrowid

            # Now fetch the inserted row
            cursor.execute("SELECT * FROM status WHERE id = %s", (inserted_id,))
            row = cursor.fetchone()
    ...
    def handle_put(self):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("UPDATE status SET name = %s WHERE id = %s", (name, status_id))
            conn.commit()
