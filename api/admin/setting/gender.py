...
class GenderHandler:
   ...
   #Read JSON Body
    ...
    def _get_id(self):
        ...
        if len(parts) == 3 and parts[0] == "admin" and parts[1] == "gender" and parts[2].isdigit():
            return int(parts[2])
        return None
    ...
    def handle_get(self):
        ...
        try:
            ...
            gender_id = self._get_id()

            #  Get all
            if path == "/admin/gender":
            ...
            # Get by ID
                ...
                cursor.execute("SELECT * FROM gender WHERE id = %s", (gender_id,))
                row = cursor.fetchone()
                if row:
    ...
    def handle_post(self):
        if self.request_handler.path != "/admin/gender":
            return 404, {"Content-Type": "application/json"}, json.dumps({"error": "Invalid URL"}).encode()
        ...
        try:
            ...
            cursor.execute("SELECT * FROM gender WHERE id = %s", (inserted_id,))
            row = cursor.fetchone()
    ...
    def handle_put(self):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("UPDATE gender SET name = %s WHERE id = %s", (name, gender_id))
            conn.commit()
    ...
    def handle_delete(self):
        ...
        try:
            ...
            # Delete the record
            cursor.execute("DELETE FROM gender WHERE id = %s", (gender_id,))
            conn.commit()
